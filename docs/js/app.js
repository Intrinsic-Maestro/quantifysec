...
async function fetchOptimizationData() {
    try {
        const response = await fetch('https://quantifysec-production.up.railway.app/api/run-pipeline', {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer fake-token-for-local-testing'
            }
        });
        ...
        console.log("RAW API RESPONSE:", apiData); // TEMP: check this in browser console before removing

        const opt = apiData.cfo_budget_optimization || {};
        Object.assign(KNAPSACK_DATA, {
            selected_controls: opt.selected_controls || [],
            deferred_controls: opt.deferred_controls || [],
            total_cost: opt.total_cost || 0,
            total_risk_reduction: opt.total_risk_reduction || 0,
            budget: opt.budget || KNAPSACK_DATA.budget,
            status: opt.status,
            solver_time_seconds: opt.solver_time_seconds,
        });
        ...
        console.error("Telemetry Sync Failed. Is the Railway backend running?", error);
    }
}
